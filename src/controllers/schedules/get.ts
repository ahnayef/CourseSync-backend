...
const getSchedule = async (req: any, res: any) => {
    ...
    try {
        ...
        if (user.role == "hod") {
            ...
            const [rows]: any = await db.query("SELECT s.id, s.day, s.department, s.session, s.start, s.end, s.room, courses.name AS name, courses.code AS courseCode,  users.name AS teacher FROM schedules AS s INNER JOIN users ON users.id = courses.instructor INNER JOIN courses ON courses.id = s.course INNER JOIN enroll ON enroll.course_id = s.course WHERE enroll.student_id = ? ORDER BY s.start", [user.id]);
